...
double closest_obstacle=1;
double closest_obstacle_angle;
...
bool canSwitchObs=false;

// recive waypoint to go to
...
void closestObstacleCallback(const geometry_msgs::PointStamped::ConstPtr& msg)
{
  closest_obstacle=msg->point.x;
  if(closest_obstacle>1.5)
  {
    closest_obstacle=1.5;
  }
  closest_obstacle_angle=msg->point.y;
  uturn_radius=0.3*closest_obstacle;
  // std::cout << "closest_obs data:" <<closest_obstacle<<","<<closest_obstacle_angle<<'\n';
}
...
int main(int argc, char **argv)
{
  ...
  throttle_pub = n.advertise<std_msgs::Float64>("/throttle/command", queue_size);
  steering_pub = n.advertise<std_msgs::Float64>("/steering/command", queue_size);
  marker_pub = n.advertise<visualization_msgs::Marker>("uturn_marker", queue_size);

  ros::Subscriber goal_sub = n.subscribe("/waypoint", 5, goalCallback);
  ...
  nh_rel.param("throttle_max",speed_max,0.5);
      ...
      else
      {
			  ...
			  if(fabs(theta)>uturn_theta_threshold+uturn_theta_threshold_hysteresis)
        {
  				...
  				if(mode==NORMAL)
          {
            ...
            listener.transformPoint("map",zero,uturn_center); // coord of base_link zero in map frame =uturn_center
			  ...
			  switch(mode)
        {
					...
					if(goalInOdom.point.x*goalInOdom.point.x+goalInOdom.point.y*goalInOdom.point.y>(uturn_radius+uturn_radius_hysteresis)*(uturn_radius+uturn_radius_hysteresis))
          {
						if(canSwitch)
            {
              std::cout << "switching uturn direction" << std::endl;
							throttle_pub.publish(zero);
							ros::Duration dur(.5);
							dur.sleep();
							current_turn*=-1;
          ...
          // if((closest_obstacle<.6)&&(abs(closest_obstacle_angle*180/3.1414)<60))
          // {
					// 	if(canSwitchObs)
          //   {
          //     std::cout << "switching uturn direction:obs too close" << std::endl;
					// 		throttle_pub.publish(zero);
